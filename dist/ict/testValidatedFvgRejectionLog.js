...
const validatedFvgRejectionLog_1 = require("./validatedFvgRejectionLog");
...
function test1RejectedFvgRecordsNoSweepReason() {
    const result = rejectedResultWith({ liquiditySweep: false });
    const record = (0, validatedFvgRejectionLog_1.toRejectedFvgRecord)(result, { symbol: 'BTC', timestamp: 'T0' });
    return {
...
function test2RejectedFvgRecordsNoDisplacementReason() {
    ...
    return {
        ...
        passed: !!record
            ...
            && record.bodyToRangeRequiredPercent === validatedFvgRejectionLog_1.BODY_TO_RANGE_PERCENT_REQUIRED
            && record.rangeMultipleRequired === validatedFvgRejectionLog_1.RANGE_MULTIPLE_REQUIRED
            && record.failedChecks.includes('displacement'),
    };
}
function test3RejectedFvgRecordsNoMssReason() {
...
function test4AcceptedFvgIsNotLoggedAsRejected() {
    const result = acceptedResult();
    const inMemoryRecord = (0, validatedFvgRejectionLog_1.toRejectedFvgRecord)(result, { symbol: 'BTC', timestamp: 'T0' });
    const log = new validatedFvgRejectionLog_1.ValidatedFvgRejectionLog({ logsDir: tmpLogsDir });
...
function test5SummaryCountsAreCorrect() {
    const batch = [
    ...
    const summary = (0, validatedFvgRejectionLog_1.summarizeValidationResults)(batch);
...
function rejectedResultWith(failed) {
    ...
    if (!liquiditySweepPassed)
        rejectionReasons.push('Bullish FVG did not sweep sell-side liquidity first');
    if (!displacementPassed)
        rejectionReasons.push('FVG lacks required bullish displacement');
    if (!mssPassed)
        rejectionReasons.push('Bullish FVG did not close above prior structure high');
    if (!premiumDiscountPassed)
        rejectionReasons.push('FVG is not in required discount context');
    if (!sessionPassed)
        rejectionReasons.push('FVG formed outside allowed UTC hours');
    const validation = {
        accepted: false,
        rawFvgId: 'test-fvg',
        direction: 'BULLISH',
