...
exports.evaluateZoneReaction = void 0;
...
const DEFAULT_DISPLACEMENT_OPTIONS = {
    enabled: true,
    lookback: 5,
    bodyToRangeMin: 0.6,
    rangeMultiplier: 1.2,
...
function evaluateReaction(input) {
    ...
    let winner = tier.winner;
    let score = tier.score;
    if (!volumeAllowed && winner !== 'NONE') {
        winner = 'NONE';
        score = SCORE_NONE;
    }
    const output = winner === 'BUY' || winner === 'SELL' ? winner : 'NONE';
    const reactionKind = winner === 'BUY'
        ? 'BULLISH_REACTION'
        : winner === 'SELL'
            ? 'BEARISH_REACTION'
            : 'NO_REACTION';
    ...
    const checks = {
        returnToZone: makeCheck(touchedZone, touchedZone ? 'Price traded into the zone range' : 'Price has not returned to the zone range'),
        midpointInteraction: makeCheck(touchedMidpoint, touchedMidpoint
...
function evaluateTier(params) {
    ...
    const midpointResult = !touchedMidpoint
        ...
        : close > zone.midpoint
            ...
            : close < zone.midpoint
                ? 'BEARISH'
                : 'NEUTRAL';
    const boundaryCloseResult = close > zone.high
        ? 'BULLISH'
        : close < zone.low
            ? 'BEARISH'
            : 'NEUTRAL';
    ...
    const displacementReaction = displacementMet && close > zone.high
        ? 'BULLISH'
...
function candleTouchesZone(zone, candle, currentPrice) {
...
function candleTouchesMidpoint(zone, candle, currentPrice) {
...
function isDisplacementCandle(candle, candles, options) {
    const range = candle.high - candle.low;
    const body = Math.abs(candle.close - candle.open);
    if (range <= 0)
        return false;
    const bodyToRange = body / range;
    if (bodyToRange < options.bodyToRangeMin)
        return false;
...
function evaluateVolumeConfirmation(candles, options) {
    if (!options.enabled) {
        return {
...
function noReaction(input, latestCandle, reason) {
    const failedCheck = {
        status: 'FAIL',
        passed: false,
        detail: reason,
    };
    const notEvaluatedCheck = {
        status: 'NOT_EVALUATED',
        passed: false,
...
function makeCheck(passed, detail) {
    return {
        status: passed ? 'PASS' : 'FAIL',
        passed,
        detail,
...
function mergeVolumeOptions(options) {
    return { ...DEFAULT_VOLUME_OPTIONS, ...(options?.volume ?? {}) };
}
function mergeDisplacementOptions(options) {
