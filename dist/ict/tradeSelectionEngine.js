...
exports.selectIctTradeCandidate = void 0;
exports.selectTradeCandidate = selectTradeCandidate;
...
const DEFAULT_EXIT_TARGET_MODE = 'HYBRID';
...
function selectTradeCandidate(input) {
    ...
    const updatedCandidates = candidates.map((candidate) => {
        if (!selected || candidate.status === 'REJECTED')
            return candidate;
...
exports.selectIctTradeCandidate = selectTradeCandidate;
function buildCandidate(evaluation, input, options) {
...
function partialReactionScore(evaluation) {
...
function isReactionConfirmed(evaluation) {
...
function distanceToInvalidationPercent(evaluation, currentPrice) {
...
function classifyTargetFit(expectedProfitUsd, options) {
...
function reactionTierScoreOf(tier) {
    switch (tier) {
...
function displacementScoreOf(evaluation) {
    const validation = evaluation.zone.validation;
...
function scalpTargetFitScoreOf(expectedProfitUsd, options) {
    ...
    if (expectedProfitUsd >= widerLow && expectedProfitUsd <= widerHigh)
        return SCALP_FIT_NEAR;
    return 0;
}
function zoneFreshnessScoreOf(evaluation, currentBarIndex) {
    if (currentBarIndex === null)
        return ZONE_FRESHNESS_UNKNOWN;
    const formed = formedBarIndexOf(evaluation);
    if (formed === null)
        return ZONE_FRESHNESS_UNKNOWN;
    const age = Math.max(0, currentBarIndex - formed);
    if (age <= 5)
        return ZONE_FRESHNESS_MAX;
    if (age <= 20)
        return 10;
    if (age <= 50)
        return 5;
...
function formedBarIndexOf(evaluation) {
    const zone = evaluation.zone;
    if (typeof zone.inversionCandleIndex === 'number')
        return zone.inversionCandleIndex;
    if (typeof zone.candle3Index === 'number')
        return zone.candle3Index;
    return null;
}
function targetDistancePenaltyOf(expectedMovePercent, exitTargetMode) {
    // SCALP-mode targets are deterministically close (1.5R) — no penalty.
    if (exitTargetMode === 'SCALP')
        return 0;
    if (!Number.isFinite(expectedMovePercent) || expectedMovePercent <= 0)
        return 0;
    if (expectedMovePercent <= 1)
        return 0;
    if (expectedMovePercent <= 2)
        return 8;
    if (expectedMovePercent <= 5)
        return 18;
    return DISTANCE_PENALTY_MAX;
}
function estimateTimeToTarget(rewardDistance, riskDistance) {
