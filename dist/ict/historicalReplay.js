...
exports.ICT_REPLAY_REPORT_PATH = void 0;
exports.runHistoricalReplay = runHistoricalReplay;
...
exports.ICT_REPLAY_REPORT_PATH = path.join(LOGS_DIR, 'ict-replay-report.json');
...
function runHistoricalReplay(inputPaths) {
    const resolvedInputs = resolveInputFiles(inputPaths);
    if (resolvedInputs.length === 0) {
        throw new Error(`No historical candle files found. Pass CSV/JSON files or place files in ${DEFAULT_INPUT_DIR}.`);
    }
    const analyses = resolvedInputs.map(inputPath => replayFile(inputPath));
    const report = {
        generatedAt: new Date().toISOString(),
        inputPaths: resolvedInputs,
        outputPath: exports.ICT_REPLAY_REPORT_PATH,
        files: analyses.map(analysis => analysis.report),
        totals: aggregateReports(analyses),
    };
    ensureLogsDir();
    fs.writeFileSync(exports.ICT_REPLAY_REPORT_PATH, JSON.stringify(report, null, 2), 'utf-8');
    return report;
}
...
function replayFile(inputPath) {
    const candles = readCandles(inputPath);
    const fvgs = (0, fvgDetector_1.detectFVGs)(candles);
    const ifvgs = (0, ifvgDetector_1.detectIFVGs)(fvgs, candles);
...
function buildStats(candleCount, fvgs, ifvgs, fvgTimings, ifvgTimings, fvgReactions, ifvgReactions) {
...
function aggregateReports(analyses) {
    ...
    return {
        ...
        combinedFillRate: rate([...allFvgTimings, ...allIfvgTimings].filter(timing => timing.filled).length, allFvgTimings.length + allIfvgTimings.length),
        ...
        combinedFlipRate: rate([...allFvgTimings, ...allIfvgTimings].filter(timing => timing.flipped).length, allFvgTimings.length + allIfvgTimings.length),
        ...
        averageCombinedLifespanCandles: average([...allFvgTimings, ...allIfvgTimings].map(timing => timing.lifespanCandles)),
...
function analyzeFVGTiming(zone, candles) {
    let fillIndex = null;
    let flipIndex = null;
    for (let i = zone.candle3Index + 1; i < candles.length; i++) {
        ...
        if (!candle)
            continue;
        if (fillIndex === null && isFVGFill(zone, candle))
            fillIndex = i;
        if (flipIndex === null && isFVGFlip(zone, candle))
            flipIndex = i;
        if (fillIndex !== null && flipIndex !== null)
            break;
    }
    return zoneTiming('FVG', zone.direction, zone.candle3Index, fillIndex, flipIndex, candles);
}
function analyzeIFVGTiming(zone, candles) {
    let fillIndex = null;
    let flipIndex = null;
    for (let i = zone.inversionCandleIndex + 1; i < candles.length; i++) {
        ...
        if (!candle)
            continue;
        if (fillIndex === null && isIFVGFill(zone, candle))
            fillIndex = i;
        if (flipIndex === null && isIFVGFlip(zone, candle))
            flipIndex = i;
        if (fillIndex !== null && flipIndex !== null)
            break;
    }
    return zoneTiming('IFVG', zone.direction, zone.inversionCandleIndex, fillIndex, flipIndex, candles);
}
function analyzeFVGReaction(zone, candles) {
    return analyzeZoneReaction('FVG', zone, zone.candle3Index, candles);
}
function analyzeIFVGReaction(zone, candles) {
    return analyzeZoneReaction('IFVG', zone, zone.inversionCandleIndex, candles);
}
function analyzeZoneReaction(type, zone, createdIndex, candles) {
    let latestNone = null;
    ...
    for (let i = createdIndex + 1; i < candles.length; i++) {
        ...
        if (!candle)
            continue;
        const result = (0, reactionEngine_1.evaluateReaction)({
            zone: reactionZone,
...
function reactionObservation(type, zone, result) {
    return {
        type,
        direction: zone.direction,
...
function reactionAnalytics(observations) {
    ...
    return {
        ...
        buyConfidenceDistribution: confidenceDistributionFor(observations.filter(observation => observation.output === 'BUY').map(observation => observation.confidence)),
        sellConfidenceDistribution: confidenceDistributionFor(observations.filter(observation => observation.output === 'SELL').map(observation => observation.confidence)),
        noneConfidenceDistribution: confidenceDistributionFor(observations.filter(observation => observation.output === 'NONE').map(observation => observation.confidence)),
...
function signalObservation(observation) {
    const signal = (0, ictSignalEngine_1.createIctSignal)({
        zone: observation.zone,
        reaction: observation.reactionResult,
        options: {
            minConfidence: ictSignalEngine_1.DEFAULT_ICT_SIGNAL_MIN_CONFIDENCE,
        },
    });
    return {
...
function signalAnalytics(observations) {
    ...
    return {
        ...
        confidenceDistribution: confidenceDistributionFor(observations.map(observation => observation.confidence)),
        buyConfidenceDistribution: confidenceDistributionFor(observations.filter(observation => observation.signal === 'BUY').map(observation => observation.confidence)),
        sellConfidenceDistribution: confidenceDistributionFor(observations.filter(observation => observation.signal === 'SELL').map(observation => observation.confidence)),
        noneConfidenceDistribution: confidenceDistributionFor(observations.filter(observation => observation.signal === 'NONE').map(observation => observation.confidence)),
        signalsByZoneType: {
...
function signalTypeCounts(observations) {
    return {
...
function isConfidenceRejection(observation) {
...
function zoneTiming(type, direction, createdIndex, fillIndex, flipIndex, candles) {
...
function readCandles(inputPath) {
...
function parseJsonCandles(raw, inputPath) {
...
function parseCsvCandles(raw, inputPath) {
    const lines = raw
        .split(/\r?\n/)
        .map(line => line.trim())
...
function candleFromArray(row, inputPath, index) {
    if (row.length < 6) {
...
function candleFromRecord(record, inputPath, index) {
...
function validateCandle(candle, inputPath, index) {
...
function collectSupportedFiles(directory, files) {
...
function distributionSet(fvgTimings, ifvgTimings, key) {
...
function valuesFor(timings, key) {
    return timings
...
function confidenceDistributionFor(values) {
    const sorted = [...values].sort((a, b) => a - b);
    return {
        count: sorted.length,
        min: sorted.length > 0 ? round(sorted[0]) : 0,
...
function bucketConfidence(values) {
    const buckets = {
        '0': 0,
        '1-25': 0,
        '26-50': 0,
...
function isSupportedCandleFile(filePath) {
    const ext = path.extname(filePath).toLowerCase();
    return ext === '.csv' || ext === '.json';
}
function asRecord(value, inputPath, index) {
...
function formatDistribution(distributionValue) {
