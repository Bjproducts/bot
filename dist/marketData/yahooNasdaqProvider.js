...
exports.YahooNasdaqProvider = void 0;
class YahooNasdaqProvider {
    providerName = 'Yahoo Finance';
    supportedSymbols = ['QQQ', 'NDX', 'NQ'];
    async fetchLatestClosedCandle(symbol) {
    ...
    toYahooSymbol(symbol) {
        ...
        if (!yahooSymbol) {
            throw new Error(`NASDAQ_PUBLIC mode: symbol "${symbol}" is not supported by the Yahoo provider. ` +
                `Supported: ${Object.keys(map).join(', ')}.`);
        }
        return yahooSymbol;
    }
}
exports.YahooNasdaqProvider = YahooNasdaqProvider;
function latestCompleteCandle(timestamps, quote) {
