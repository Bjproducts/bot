...
import { repairTradesCsvHeader } from '../journal/tradeJournal';
...
export function generateLogViewer(): string {
  ensureLogsDir();
  const csvRepair = repairTradesCsvHeader(LOGS_DIR);
  const completedTrades = readJson<CompletedTrade[]>('logs/completed-trades.json', []);
  const ictAudit = readJson<IctSignalAuditFile>('logs/ict-signals.json', {});
  const sessionStats = readJson<Record<string, unknown>>('session-stats.json', {});
  const positionState = readJson<Record<string, unknown>>('position-state.json', {});
...
function fileRow(file: { label: string; relativePath: string }): string {
...
function rawFileDetails(file: { label: string; relativePath: string }): string {
  const filePath = path.join(ROOT_DIR, file.relativePath);
...
function completedTradesTable(trades: CompletedTrade[]): string {
  if (trades.length === 0) {
...
function card(label: string, value: string, className: string = ''): string {
