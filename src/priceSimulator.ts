...
export class PriceSimulator {
  ...
  private momentumTicks:     number = 0;
  private momentumDirection: number = 0;

  constructor(
    startPrice:      number,
    annualisedVol:   number,
    annualisedDrift: number,
    tickIntervalMs:  number,
    baseVolume:      number = 800,
  ) {
  ...
  nextCandle(): Candle {
    ...
    const momentumBias = this.momentumTicks > 0
      ...
      : 0;
    if (this.momentumTicks > 0) this.momentumTicks--;
    ...
    const volume           = Math.round(
      this.baseVolume * baseNoise * returnAmplifier * independentSpike,
    );
