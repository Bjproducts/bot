...
export type ExitTargetMode = 'STRUCTURE' | 'SCALP' | 'HYBRID';
export type ManagedTargetSource = 'OPPOSING_FVG' | 'SWING' | 'PERCENT' | 'SCALP_R';
...
export interface TargetSelectionConfig {
  ...
  // 0 = disabled; otherwise reject structure targets farther than X% from entry.
...
export interface TargetSelectionInput {
  ...
  entryPrice: number;
  stopPrice: number;
  opposingZones: readonly IctSignalZone[];
  swingTargetPrice: number | null;
  config: TargetSelectionConfig;
}

export interface TargetSelectionResult {
  exitTargetMode: ExitTargetMode;
  structureTarget: ManagedTarget | null;
  scalpTarget: ManagedTarget | null;
  selectedTarget: ManagedTarget | null;
  selectedTargetReason: string;
  targetRMultiple: number;
  structureRiskRewardRatio: number | null;
  scalpRiskRewardRatio: number | null;
}

export function selectManagedTarget(input: TargetSelectionInput): TargetSelectionResult {
  ...
  if (!scalpTarget) {
    return summarize(
      ...
      structureTarget,
      null,
      structureValid ? structureTarget : null,
      structureValid
        ? `HYBRID mode: scalp unavailable, using structure ${structureTarget!.source}`
        : 'HYBRID mode: scalp unavailable and structure fails RR / distance gate',
      targetRMultiple,
      structureRR,
      scalpRR,
    );
  ...
  if (!structureTarget) {
    return summarize(
      ...
      scalpTarget,
      scalpTarget,
      `HYBRID mode: no structure target, using scalp ${targetRMultiple.toFixed(2)}R`,
      targetRMultiple,
      structureRR,
      scalpRR,
    );
  ...
  if (structureDistance <= scalpDistance) {
    return summarize(
      ...
      scalpTarget,
      structureTarget,
      `HYBRID mode: structure ${structureTarget.source} closer than scalp (RR ${structureRR!.toFixed(2)})`,
      targetRMultiple,
      structureRR,
      scalpRR,
    );
  ...
  return summarize(
    ...
    scalpTarget,
    scalpTarget,
    `HYBRID mode: scalp ${targetRMultiple.toFixed(2)}R closer than structure`,
    targetRMultiple,
    structureRR,
    scalpRR,
  );
...
export function findStructureTarget(
  ...
  if (opposing) return opposing;
  if (swingTargetPrice === null || !Number.isFinite(swingTargetPrice)) return null;

  // Swing target must be in profit direction.
  if (side === 'LONG' && swingTargetPrice <= entryPrice) return null;
  if (side === 'SHORT' && swingTargetPrice >= entryPrice) return null;

  return { price: swingTargetPrice, source: 'SWING' };
}

export function findOpposingZoneTarget(
  side: 'LONG' | 'SHORT',
  entryPrice: number,
  opposingZones: readonly IctSignalZone[],
): ManagedTarget | null {
...
export function findScalpTarget(
  side: 'LONG' | 'SHORT',
  entryPrice: number,
  stopPrice: number,
  targetRMultiple: number,
): ManagedTarget | null {
  if (!isFinitePositive(entryPrice) || !isFinitePositive(stopPrice)) return null;
  if (!Number.isFinite(targetRMultiple) || targetRMultiple <= 0) return null;
...
function computeRiskReward(targetPrice: number | null, entryPrice: number, stopPrice: number): number | null {
...
function structureWithinMaxDistance(
  target: ManagedTarget,
  entryPrice: number,
  maxPercent: number,
): boolean {
  if (maxPercent <= 0) return true;
  if (!isFinitePositive(entryPrice)) return true;
