...
export function calculatePositionSizing(input: PositionSizingInput): PositionSizingResult {
  ...
  const minRiskRewardRatio = Number.isFinite(config.minRiskRewardRatio)
    ? (config.minRiskRewardRatio as number)
  ...
  const targetRMultiple = Number.isFinite(config.targetRMultiple)
    ? (config.targetRMultiple as number)
  ...
  if (expectedProfitUsd < config.targetProfitMinUsd) {
    return rejected(
      input,
      `Expected profit $${expectedProfitUsd.toFixed(2)} < minimum $${config.targetProfitMinUsd.toFixed(2)} (position $${boundedSize.toFixed(2)}, risk ${riskDistance.toFixed(4)}, reward ${rewardDistance.toFixed(4)})`,
      {
        expectedMovePercent,
        rewardDistance,
        riskDistance,
...
function calculateRiskFirst(
  ...
  if (expectedProfitUsd < config.targetProfitMinUsd) {
    return rejected(
      input,
      `Expected profit $${expectedProfitUsd.toFixed(2)} < minimum $${config.targetProfitMinUsd.toFixed(2)} (position $${boundedSize.toFixed(2)}, risk ${riskDistance.toFixed(4)}, reward ${resolvedRewardDistance.toFixed(4)})`,
      {
        expectedMovePercent: resolvedExpectedMovePercent,
        rewardDistance: resolvedRewardDistance,
        riskDistance,
...
function confidenceSizeMultiplier(confidence: number): number {
  if (confidence >= 95) return 1.15;
  if (confidence >= 85) return 1.00;
  if (confidence < 80) return 0.75;
  return 0.90;
}

function scoreSizeMultiplier(score: number): number {
...
function riskUtilization(expectedLossUsd: number, maxRiskPerTradeUsd: number): number {
