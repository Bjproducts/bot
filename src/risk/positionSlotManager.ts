...
export type SlotClassification = 'PROTECTED' | 'RISK';

export interface PositionSlotInput {
  id: string;
  stopAtBreakeven: boolean;
  partialCloseDone?: boolean;
  /** Current active stop price; if equal to averageEntryPrice the
   *  position is treated as protected even if stopAtBreakeven isn't
   *  flipped yet. */
  activeStopPrice?: number | null;
  averageEntryPrice?: number;
}

export const DEFAULT_MAX_TOTAL_OPEN_POSITIONS = 5;
export const DEFAULT_MAX_ACTIVE_RISK_POSITIONS = 3;

export function classifyPositionSlot(p: PositionSlotInput): SlotClassification {
  if (p.stopAtBreakeven === true) return 'PROTECTED';
  if (
    typeof p.activeStopPrice === 'number'
...
export interface PositionSlotCounts {
  total: number;
  risk: number;
  protected: number;
}

export function countPositionSlots(positions: readonly PositionSlotInput[]): PositionSlotCounts {
  let total = 0;
  let risk = 0;
  for (const p of positions) {
    total++;
    if (classifyPositionSlot(p) === 'RISK') risk++;
  }
  return { total, risk, protected: total - risk };
}

export type SlotBlockReasonCode =
  | 'NONE'
  | 'MAX_TOTAL_POSITIONS'
  | 'MAX_RISK_POSITIONS';

export interface PositionSlotEvaluation {
  totalOpen: number;
  riskCount: number;
  ...
  blockReasonCode: SlotBlockReasonCode;
  blockReason: string;
}

export function evaluatePositionSlotGate(
  positions: readonly PositionSlotInput[],
  opts: { maxTotal?: number; maxRisk?: number } = {},
): PositionSlotEvaluation {
  const maxTotal = opts.maxTotal ?? DEFAULT_MAX_TOTAL_OPEN_POSITIONS;
  const maxRisk = opts.maxRisk ?? DEFAULT_MAX_ACTIVE_RISK_POSITIONS;
