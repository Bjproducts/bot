...
import { TradeCandidate, TradeSelectionResult } from './tradeCandidateTypes';
import {
  DEFAULT_MAX_GAP_SECONDS,
  clearIctStateForGap,
  detectCandleGap,
} from './candleBufferGap';
...
function test1NoResetOnNormalSequence(): TestResult {
  ...
  const result = detectCandleGap(prev, next);
  return {
    name: 'no reset on normal sequence (60s delta)',
    expected: `gapDetected=false, gapSeconds=60, threshold=${DEFAULT_MAX_GAP_SECONDS}`,
    ...
    passed: result.gapDetected === false
      ...
      && result.thresholdSeconds === DEFAULT_MAX_GAP_SECONDS,
  };
}

function test2ResetOnLargeGap(): TestResult {
...
function test3FvgStateCleared(): TestResult {
  const state = buildPopulatedState({ fvgCount: 3, ifvgCount: 0, candidates: 0 });
  const summary = clearIctStateForGap(state);
...
function test4IfvgStateCleared(): TestResult {
...
function test5CandidateCacheCleared(): TestResult {
...
function buildPopulatedState(opts: { fvgCount: number; ifvgCount: number; candidates: number }) {
  const ictCandleBuffer: Candle[] = [];
  for (let i = 0; i < 10; i++) {
  ...
  const latestIctZones: IctSignalZone[] = [];
  ...
  const latestTradeSelection: TradeSelectionResult = {
    action: 'NONE',
...
function buildFvgZone(id: string): IctSignalZone {
  return {
...
function buildIfvgZone(id: string): IctSignalZone {
  return {
...
function buildCandidateStub(id: string): TradeCandidate {
  return {
    signal: {
