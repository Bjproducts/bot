...
import { TargetSelectionResult } from '../risk/targetSelection';

interface ExpectedSelection {
  action: TradeSelectionResult['action'];
  selectedZoneId: string | null;
  expectedProfitAtTPUsd?: number;
  targetFit?: string;
}

interface TradeSelectionFixture {
  name: string;
  evaluations: Array<{
...
const fixtures: TradeSelectionFixture[] = [
  ...
  },
  {
    ...
    evaluations: [
      evaluation(
        fvgZone('high-probability-fvg', 'BULLISH'),
        'BUY',
        82,
        false,
        {
      ...
      evaluation(
        ifvgZone('higher-score-lower-probability-ifvg', 'BULLISH'),
        'BUY',
        100,
        true,
        {
...
function summarize(selection: TradeSelectionResult): ExpectedSelection {
...
function reaction(
  ...
  confidence: number,
  volumeConfirmed: boolean,
  reactionType: IctReactionResult['reactionType'] = action === 'NONE' ? 'NONE' : 'BOUNDARY',
): IctReactionResult {
  return {
    zoneId: zone.id,
...
function targetSelection(
  targetPrice: number,
  mode: TargetSelectionResult['exitTargetMode'],
...
function expected(
  action: ExpectedSelection['action'],
  selectedZoneId: string | null,
  expectedProfitAtTPUsd?: number,
  targetFit?: string,
): ExpectedSelection {
