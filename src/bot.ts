...
import { calculateUnrealizedPnl, evaluatePositionLifecycleExit } from './positionExitManager';
import {
  activateDollarBreakeven,
  applyPartialClose,
  getActiveStopPrice,
  planPartialClose,
...
export class BotEngine {
  ...
  private recentOppositeSignalExpiredLogged: boolean = false;
  ...
  private async bootstrapAndSchedule(): Promise<void> {
    await this.preloadStartupCandles();
    if (this.running) this.scheduleTick();
  }

  private async preloadStartupCandles(): Promise<void> {
    if (typeof this.dataSource.startupCandles !== 'function') return;
  ...
  private processIctSignal(): void {
    ...
    const oppositeProtection = this.applyOppositeSignalProtection(
      side,
      this.lastPrice,
      tradeSelection.action,
      selectedCandidate,
    );
    if (oppositeProtection.closedProfitPosition) {
      logEvent(
        'ENTRY_SKIP_OPPOSITE_PROFIT_EXIT',
        side,
        this.tick,
        'opposite profitable position closed; storing signal watch and blocking same-tick reversal entry',
      );
      return;
    }
  ...
  private managePosition(position: PositionState, price: number, candle: Candle | null): void {
    ...
    latestPosition = this.partialCloseIfEligible(
      latestPosition,
      price,
      candle?.timestamp ?? new Date(),
    );
  ...
  private activateBreakEvenIfEligible(
    ...
    if (!shouldActivateDollarBreakeven(position, price, {
      breakevenTriggerProfitUsd: this.config.breakevenTriggerProfitUsd,
    })) return position;

    const activated = activateDollarBreakeven(position, price, activationTime);
    const activationIso = activated.breakevenActivationTime ?? activationTime.toISOString();
    const unrealizedPnlUsd = calculateUnrealizedPnl(position, price);
    ...
    logEvent(
      ...
      position.side,
      this.tick,
      `BE Activated  positionId=${position.id ?? '--'}  trigger=$${this.config.breakevenTriggerProfitUsd.toFixed(2)}` +
  ...
  private partialCloseIfEligible(
    position: PositionState,
    price: number,
    closeTime: Date,
  ): PositionState {
    const plan = planPartialClose(position, price, {
    ...
    logEvent(
      ...
      position.side,
      this.tick,
      `positionId=${position.id ?? '--'}  entry=$${fp(position.averageEntryPrice)}` +
  ...
  private applyOppositeSignalProtection(
    ...
    for (const position of [...this.positions]) {
      ...
      const plan = planOppositeSignalProtection(
        ...
        price,
        newSignalSide,
        this.config.oppositeSignalMaxLossUsd,
      );
  ...
  private storeRecentOppositeSignalWatch(
    ...
    if (!this.config.recentSignalWatchEnabled) return;
    const now = new Date();
    this.recentOppositeSignalExpiredLogged = false;
    this.stats = {
      ...this.stats,
      ...createRecentSignalWatch({
        side: signalDirection === 'BUY' || signalDirection === 'SELL' ? signalDirection : candidate.signalDirection as 'BUY' | 'SELL',
  ...
  private updateRecentSignalWatchValidity(tradeSelection: TradeSelectionResult): void {
    if (!this.stats.recentOppositeSignalSide || !this.stats.recentOppositeSignalZoneId) return;
    ...
    if (!this.recentOppositeSignalExpiredLogged) {
      logEvent(
        'RECENT_SIGNAL_EXPIRED',
        this.stats.recentOppositeSignalSide,
        this.tick,
        `zone=${this.stats.recentOppositeSignalZoneId} age=${result.ageCandles} valid=${result.valid ? 'YES' : 'NO'}`,
      );
  ...
  private closePosition(
    ...
    logEvent(
      ...
      activeSide,
      this.tick,
      `positionId=${position.id ?? '--'}  $${fp(price)}  PnL=${sign}$${totalPnlUsd.toFixed(2)} (${sign}${pnlPct.toFixed(3)}%)  ` +
  ...
  private makeEvent(
    ...
    const unrealizedPnlUsd = position.side === 'NONE'
      ? 0
      : calculateUnrealizedPnl(position, price);
    ...
    return {
      ...
      ...this.makePositionSizingFields(position),
      ...this.makeScoreAttributionFields(position),
      activeStopPrice: getActiveStopPrice(position) ?? undefined,
      unrealizedPnlUsd,
...
function closeReasonLabel(reason: PositionCloseReason): string {
  ...
  if (reason === 'OPPOSITE_SIGNAL_PROFIT_EXIT') return 'OPPOSITE_SIGNAL_PROFIT';
